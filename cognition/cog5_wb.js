...
// Module version of your Emoji Span (working memory) page.
...
export function init(container, api) {
  ...
  function stopToIdle() {
    ...
    el.subtitle.textContent = "A short attention and working-memory test using food emojis. Two trials per length.";
  ...
  function finishAndSave(reason = "completed") {
    ...
    const session = {
      ...
      completedAt: nowISO(),
      settings: { speedMs: CONFIG.speedMs, gapMs: CONFIG.gapMs, practiceTrials: CONFIG.practiceTrials, trialsPerLength: CONFIG.trialsPerLength },
      summary: {
  ...
  function submitAnswer() {
    ...
    const correct = state.answer.join("|") === state.expected.join("|");
    state.totalTrials += 1;
  ...
  el.btnStart.addEventListener("click", () => {
    if (state.isRunning) { stopToIdle(); startTest(); }
    else { startTest(); }
  });

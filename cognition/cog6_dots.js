...
// Module version of your Dot Comparison (numerosity) test.
...
export function init(container, api) {
  ...
  /********************************************************************
   * Logic (your original, adapted + battery save on finish)
   ********************************************************************/
  const CONFIG = {
  ...
  const ctxL = el.canLeft.getContext("2d");
  const ctxR = el.canRight.getContext("2d");
  ...
  function drawDots(ctx, count) {
    ...
    while (dots.length < count && attempts < 6000) {
      attempts++;
  ...
  function pickCounts() {
    const base = randInt(CONFIG.minDots, CONFIG.maxDots);
    const bigger = Math.max(base, Math.round(base * state.minRatio));
    const smaller = base;
  ...
  function answer(side) {
    ...
    updateStatsUI();
    setBadge(el.badgePhase, correct ? "✓" : "✕", correct ? "ok" : "bad");
  ...
  function finish(reason = "completed") {
    if (!state.running && reason !== "manual_finish") return;
    ...
    // Only store if it actually ran (or has any trials)
    if (wasRunning || attempted > 0 || reason === "manual_finish") addSession(session);
